...
void Blokkendoos::push_back(unique_ptr<Figuur>& figuur) {
    if(max_opp == nullptr){
        ...
        vector<unique_ptr<Figuur>>::push_back(move(figuur)); // voeg toe aan einde van this->vector
...
Blokkendoos::Blokkendoos(const string& bestandsnaam){
    ...
    if(!input.is_open()){
        cout << "Bestand kon niet gevonden worden." << endl;
    } else {
        string soort;
        while(input >> soort){
            if(soort == "rechthoek"){
...
int main(){
    Blokkendoos blokkendoos("s:\\Documents\\CLion Projects\\C&C++ subject\\_Cpp_lab\\33\\figuren.txt");
